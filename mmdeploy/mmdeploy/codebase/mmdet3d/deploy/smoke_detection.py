...
@MMDET3D_TASK.register_module(Task.SMOKE_DETECTION.value)
class SmokeDetection(BaseTask):
    ...
    def build_backend_model(self,
        ...
        """
        from .smoke_detection_model import build_smoke_detection_model
        ...
        model = build_smoke_detection_model(
            model_files,
            self.model_cfg,
            self.deploy_cfg,
            device=self.device,
    ...
    def create_input(
        ...
        data = []
        cam2img = [[0.0 for _ in range(4)] for _ in range(4)]
        data_ = dict(
            images=dict(
                CAM2=dict(
                    img_path=pcd,
        ...
        data[0]['inputs']['img'] = data[0]['inputs']['img'].to(self.device)

        if data_preprocessor is not None:
            collate_data = data_preprocessor(collate_data, False)
            img = collate_data['inputs']['imgs']
            inputs = [img]
        else:
